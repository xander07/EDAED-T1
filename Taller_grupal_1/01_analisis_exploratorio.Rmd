...
modification: "Jhon Jairo Serna Córdoba, Laura Flórez, Karina Jiménez, Johan Paniagua, Alex"
output: 
  html_document: 
    toc: yes
---
...
Para esta visualización usaremos las función *glimpse()* que nos ofrece información general de nuestra base de datos como cantidad de datos, cantidad de variables y algunos detalles de las mismas. Veámos:
...
Como observamos, el resumen global de la base de datos nos permite observar, en las variables cualitativas, la cantidad de registros, mientras en las variables cuantitativas, nos muestra algunas estadísticas básicas como la media, la mediana, el valor máximo y mínimo, los cuartiles, cantidad de datos faltantes, etc., ayudándonos a detectar qué variables, como la Q09_EnAreaMetropolitana o Q14_RecursosInformáticos, deberían ser de tipo Factor, por ejemplo, y no de tipo Numérico.
...
Así, con toda la información que nos brinda la visualización general de nuestra base de datos y después de definir las variables con las que vamos a trabajar, entonces podemos empezar el proceso de depuración de nuestra base de datos.

## Depuración de los datos

#### 1. Cambiamos los datos que tienen formato fecha de CHARACTER a DATE.

Este cambio lo realizamos con el fin de facilitar el manejo de las fechas, aprovechando las funciones que nos ofrece R para usar con las variables de tipo *date*.

Confirmemos que las variables que tienen formato fecha (AAAA-MM-DD), efectivamente, sean de tipo distinto a *date*. Para realizar esto, usaremos las función *str()* que nos ofrece R, sobre cada una de las variables y así observar, además del tipo, algunos detalles adicionales.
...
Ahora que confirmamos que estas variables no son del tipo DATE que queremos, modifiquemos el tipo usando la función *as.Date()* de R y confirmemos el cambio, usando nuevamente la funcioón *str()*.
...
Hay otra vatiable que también tiene tipo fecha, aunque realmente es formato fecha hora (AAAA/MM/DD hh:mm:ss). Detallemos:
...
Debido a esta "variación" en el formato y para mantenelos de la forma Fecha Hora, el cambio de tipo lo haremos usando la función *as.POSIXct()*.
...
Observemos que ahora el tipo de la variable es de POSIXct, tipo especial de R que nos permitirá trabajar de mejor manera con las variables de formato Fecha Hora.
