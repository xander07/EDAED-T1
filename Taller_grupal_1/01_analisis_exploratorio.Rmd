...
¿Los estudiantes que tienen mejor PAPA se encuentran en los semestres mas altos?
...
Para responder a esta pregunta, en primer lugar lo que hizo fue analizar que carreras están cursando los estudiantes del curso de estadistica descriptiva y exploratoria, para esto se realiza una tabla de frecuencias, contabulizar cuantas personas hay por carreras. 
...
```{r}
with(encuesta, plot(saber11, Q08_PAPA,main="Saber11 vs PAPA",xlab="Saber11", ylab="PAPA", col = as.factor(encuesta$saber11),pch=19, ylim=c(3,4.7),xlim=c(1,8)))
lines(lowess(subset(encuesta, Q10_Estrato == "Bajo")[,c("saber11")], subset(encuesta, Q10_Estrato == "Bajo")[,c("Q08_PAPA")]), col = "#CC6666", lwd = 2)
lines(lowess(subset(encuesta, Q10_Estrato == "Medio")[,c("saber11")], subset(encuesta, Q10_Estrato == "Medio")[,c("Q08_PAPA")]), col = "Black", lwd = 2)
lines(lowess(subset(encuesta, Q10_Estrato == "Alto")[,c("saber11")], subset(encuesta, Q10_Estrato == "Alto")[,c("Q08_PAPA")]), col = "Black", lwd = 2)
legend("bottomright" ,legend = c("bajo", "Medio", "Alto"), col = c("Black","#cc6666"), pch = c(19, 19), cex = 0.7, box.lwd = 1)
```
